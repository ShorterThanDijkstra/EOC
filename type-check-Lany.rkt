      ...
      (lambda (e)
        ...
        (match e
          ...
          [(Prim pred (list e1))
           ...
           [(Prim op (list e1 e2))
           #:when (memq op '(+ -) )
           (define-values (new-e1 e-ty1) (recur e1))
           (check-type-equal? e-ty1 'Any e1)
             (define-values (new-e2 e-ty2) (recur e2))
           (check-type-equal? e-ty2 'Any e2)
           (values (Prim op (list new-e1 new-e2)) 'Integer)]
          [(Exit)
           (values (Exit) '_)]
          [(Apply func args)
           (values (Apply func args) 'Any)]
