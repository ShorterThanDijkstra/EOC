...
(provide type-check-gradual
         type-check-gradual-class
         type-check-Lwhile-proxy
         type-check-Lwhile-proxy-class
         type-check-Cwhile-proxy
         type-check-Cwhile-proxy-class)
        ...
        (match e
          ...
          [(Prim op es)
           ...
           (define-values (new-es ts)
             (for/lists (exprs types) ([e es]) (recur e)))
          ...
          [(If e1 e2 e3)
           ...
           (match T1
             ['Boolean
              ...
              (values (If e1^ (make-cast e2^ T2 Tif) (make-cast e3^ T3 Tif))
                      Tif)]
             ['Any
          ...
          [(Apply e1 e2s)
           ...
           (match T1
             [`(,T1ps ... -> ,T1rt)
              ...
              (define e2s^^
                (for/list ([e2 e2s^] [src T2s] [tgt T1ps])
          ...
          [(Lambda params Tr e1)
           ...
           (values (Lambda (for/list ([x xs] [T Ts])
                             `[,x : ,T])
                           Tr
                           (make-cast e1^ T1 Tr))
                   `(,@Ts -> ,Tr))]
      ...
      (match e
        ...
        [(Prim 'inject-proxy (list e1))
         ...
         (match T1
           ...
           [`(Vector (PVector ,ts0 ...)
                     (Vector (Vector (,clos ,ts1 -> ,ts2)) ...)
                     ,ws)
            (values (Prim 'inject-proxy (list e1^)) `(PVector ,@ts2))])]
        [(Prim 'proxy? (list e1))
         (define-values (e1^ T1) (recur e1))
         (match T1
           [`(PVector ,ts ...) (values (Prim 'proxy? (list e1^)) 'Boolean)])]
        ...
        [(Prim 'proxy-vector-ref (list e1 e2))
         ...
         (match (list T1 e2^)
           [(list `(PVector ,ts ...) (Int i))
            (unless (and (0 . <= . i) (i . < . (length ts)))
              (error 'type-check "invalid index ~a in ~a" i e))
            (values (Prim 'proxy-vector-ref (list e1^ e2^)) (list-ref ts i))])]
        [(Prim 'proxy-vector-set! (list e1 e2 e3))
         ...
         (match (list T1 e2^)
           [(list `(PVector ,ts ...) (Int i))
            ...
            (values (Prim 'proxy-vector-set! (list e1^ e2^ e3^)) 'Void)])]
        [else ((super type-check-exp env) e)]))))
